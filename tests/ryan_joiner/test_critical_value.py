"""Tests if  ``critical_value`` is working as expected

--------------------------------------------------------------------------------
Command to run at the prompt:
    python -m unittest -v tests/ryan_joiner/test_critical_value.py
    or
    python -m unittest -b tests/ryan_joiner/test_critical_value.py
...
### FUNCTION IMPORT ###
from normtest.ryan_joiner import critical_value

os.system("cls")


class Test_critical_value(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
    ...
    def test_outputs(self):
        result = critical_value(self.n, self.alpha)
        self.assertIsInstance(
            result, float, msg=f"not a float when alpha={self.alpha} and n={self.n}"
        )

    def test_safe(self):
        result = critical_value(self.n, self.alpha, safe=True)
        self.assertIsInstance(
            result, float, msg=f"not a float when alpha={self.alpha} and n={self.n}"
        )
    ...
    def test_statext(self):
        ...
        result = critical_value(x_data.size, 0.01)
        ...
        result = critical_value(x_data.size, 0.05)
        self.assertAlmostEqual(
            result,
            0.9230,
            places=3,
        ...
        result = critical_value(x_data.size, 0.10)
        self.assertAlmostEqual(
            result,
            0.9387,
            places=3,
    ...
    def test_alpha_value(self):
        ...
        with self.assertRaises(
            ...
            result = critical_value(self.n, alpha, safe=True)
    ...
    def test_n_not_int(self):
        ...
        for n in n_values:
            with self.assertRaises(
                ...
                result = critical_value(n, 0.05, safe=True)
